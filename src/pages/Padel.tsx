  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-gray-900">Padel Sessions</h1>
          <p className="text-sm md:text-base text-gray-500 mt-2">Kelola sesi, kehadiran, dan pembayaran Padel.</p>
        </div>
        <button
          onClick={() => setIsModalOpen(true)}
          className="w-full md:w-auto flex justify-center items-center gap-2 bg-primary-purple hover:bg-opacity-90 text-white px-5 py-2.5 rounded-xl font-medium transition-all shadow-sm"
        >
          <Plus size={20} />
          Buat Sesi Baru
        </button>
        ...
        {sessions.map(session => {
          ...
          return (
            <div key={session.id} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
              ...
              <Link to={`/padel/sessions/${session.id}`} className="block group">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <h2 className="text-2xl font-bold mb-6">Buat Sesi Padel</h2>
            <form onSubmit={handleCreateSession} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="text" required placeholder="Nama Lapangan" className="w-full px-3 py-2 border rounded-xl"
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="block text-sm font-medium text-gray-700">Jadwal & Court</label>
                  <button type="button" onClick={addSlot}
                    className="text-xs text-primary-purple font-medium flex items-center gap-1 hover:opacity-80">
                ...
                <div className="space-y-2">
                  {sessionSlots.map((slot, i) => (
                    <div key={i} className="flex gap-2 items-center">
                      <input type="text" placeholder="12.00–15.00"
              ...
              <div className="flex gap-3 justify-end pt-4">
                ...
                <button type="submit" className="px-6 py-2 bg-primary-purple text-white rounded-xl hover:bg-opacity-90">Simpan Sesi</button>
              </div>
            </form>
          </div>
        </div>
